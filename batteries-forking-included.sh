...
if [ "${__MARXIMUS_SHELL_EXTENSIONS__GLOBAL__OPTIONS_OLD}" = "" ]; then
    ...
    __MARXIMUS_SHELL_EXTENSIONS__GLOBAL__OPTIONS_OLD="$(echo "${__MARXIMUS_SHELL_EXTENSIONS__GLOBAL__OPTIONS_OLD}" | sed 's/c//g')"
fi
...
if [ "$ret" -ne 0 ]; then
    ...
    def; trim() {
        (
            text="$1"
            echo "${text}" | sed -e '${/^[[:space:]]*$/d;}' -e '1{/^[[:space:]]*$/d;}'
        )
    }
    ...
    #-------------------------------------------------------------------------------
    def; join_lines_cleanly() {
        (
            text="$1"
            printf '%s' "$(remove_newlines "$(unindent_text "$(trim "${text}")")")"
        )
    }
...
def; ensure_conda() {
    (
        ...
        needs_conda_install=false

        log_superdebug "$(ls -alG "${CONDA_INSTALL_PATH}/etc/profile.d/conda.sh")"
        ...
        if [ "${needs_conda_install}" = false ]; then
            ...
            ret=$?
            if [ "$(call return_code_is_error "$ret")" = true ]; then
                call log_footer "Conda Could Not Be Initialized."
                needs_conda_install=true
            fi
        fi
