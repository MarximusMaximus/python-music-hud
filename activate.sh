...
if [ "${__array__SHELL_SOURCE__length}" -eq 0 ]; then
    ...
    TEMP_ARG_ZERO="${TEMP_ARG_ZERO##*[/\\]}"
    nullcall log_ultradebug "\${TEMP_ARG_ZERO}=${TEMP_ARG_ZERO}"
    nullcall log_ultradebug "which lsof=%s" "$(which lsof)"w
    nullcall log_ultradebug ""
    nullcall log_ultradebug "\n%s" "$(set | sort)"
    nullcall log_ultradebug ""
    if which sh 1>/dev/null 2>/dev/null; then
        nullcall log_ultradebug "\n%s" "$(lsof -p $$ -Fn0) 2>&1"
        nullcall log_ultradebug " "
        # shellcheck disable=SC2005
        nullcall log_ultradebug "\n%s" "$(echo "$(lsof -p $$ -Fn0)" ) 2>&1"
        nullcall log_ultradebug " "
    else
        nullcall log_ultradebug "lsof not available"
    fi
